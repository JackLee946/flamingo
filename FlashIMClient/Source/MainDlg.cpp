...
BOOL CMainDlg::OnInitDialog(CWindow wndFocus, LPARAM lInitParam)
{
	...
	{
		::MessageBox(NULL, _T("初始化代理窗口失败，FlashIM无法启动！"), _T(""), MB_OK|MB_TOPMOST);
		::ExitProcess(0);
		return FALSE;
	}

	LoadAppIcon(m_FMGClient.GetStatus());
	m_TrayIcon.AddIcon(m_hWnd, WM_TRAYICON_NOTIFY, 1, m_hAppIcon, _T("FlashIM未登录"));
...
void CMainDlg::CloseDialog(int nVal)
{
	...
	{
		if(IDNO == ::MessageBox(m_hWnd, _T("您还有文件正在传输，确认要退出FlashIM？"), g_strAppTitle.c_str(), MB_YESNO|MB_ICONQUESTION))
			return;
	}
...
BOOL CMainDlg::InitUI()
{
    ...
    CIniFile iniFile;
    CString strIniFile;
    strIniFile.Format(_T("%sconfig\\FlashIM.ini"), g_szHomePath);
...
void CMainDlg::OnBtn_Cancel(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	strMainExe.Format(_T("%sFlashIM.exe"), g_szHomePath);
