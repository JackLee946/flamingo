...
CFlashIMClient& CFlashIMClient::GetInstance()
{
    static CFlashIMClient client;
    return client;
}


CFlashIMClient::CFlashIMClient(void)
{
	m_ServerTime = 0;
...
// 初始化客户端
bool CFlashIMClient::InitProxyWnd()
{
...
bool CFlashIMClient::InitNetThreads()
{
...
void CFlashIMClient::SetServer(PCTSTR pszServer)
{
    CIUSocket::GetInstance().SetServer(pszServer);
}

void CFlashIMClient::SetFileServer(PCTSTR pszServer)
{
    CIUSocket::GetInstance().SetFileServer(pszServer);
}

void CFlashIMClient::SetImgServer(PCTSTR pszServer)
{
    CIUSocket::GetInstance().SetImgServer(pszServer);
}

void CFlashIMClient::SetPort(short port)
{
    CIUSocket::GetInstance().SetPort(port);
}

void CFlashIMClient::SetFilePort(short port)
{
    CIUSocket::GetInstance().SetFilePort(port);
}

void CFlashIMClient::SetImgPort(short port)
{
    CIUSocket::GetInstance().SetImgPort(port);
}

// 设置UTalk号码和密码
void CFlashIMClient::SetUser(LPCTSTR lpUserAccount, LPCTSTR lpUserPwd)
{
...
// 设置登录状态
void CFlashIMClient::SetLoginStatus(long nStatus)
{
	m_UserMgr.m_UserInfo.m_nStatus = nStatus;
}

// 设置回调窗口句柄
void CFlashIMClient::SetCallBackWnd(HWND hCallBackWnd)
{
	m_UserMgr.m_hCallBackWnd = hCallBackWnd;
}

void CFlashIMClient::SetRegisterWindow(HWND hwndRegister)
{
	m_hwndRegister = hwndRegister;
}

void CFlashIMClient::SetModifyPasswordWindow(HWND hwndModifyPassword)
{
	m_hwndModifyPassword = hwndModifyPassword;
}

void CFlashIMClient::SetCreateNewGroupWindow(HWND hwndCreateNewGroup)
{
	m_hwndCreateNewGroup = hwndCreateNewGroup;
}

void CFlashIMClient::Register(PCTSTR pszAccountName, PCTSTR pszNickName, PCTSTR pszPassword)
{
	//TODO: 如果此时断网，则直接返回
...
// 查找好友
BOOL CFlashIMClient::FindFriend(PCTSTR pszAccountName, long nType, HWND hReflectionWnd)
{
	//TODO: 先判断一下是否离线
...
BOOL CFlashIMClient::AddFriend(UINT uAccountToAdd)
{
	//TODO: 先判断是否离线
	if(uAccountToAdd == 0)
		return FALSE;
...
// 删除好友
BOOL CFlashIMClient::DeleteFriend(UINT uAccountID)
{
	//TODO: 先判断是否离线
	COperateFriendRequest* pRequest = new COperateFriendRequest();
...
bool CFlashIMClient::AddNewTeam(PCTSTR pszNewTeamName)
{
...
bool CFlashIMClient::DeleteTeam(PCTSTR pszOldTeamName)
{
...
bool CFlashIMClient::ModifyTeamName(PCTSTR pszNewTeamName, PCTSTR pszOldTeamName)
{
...
bool CFlashIMClient::MoveFriendToOtherTeam(UINT uUserID, PCTSTR pszOldTeamName, PCTSTR pszNewTeamName)
{
...
BOOL CFlashIMClient::UpdateLogonUserInfo(  PCTSTR pszNickName, 
										 PCTSTR pszSignature, 
										 UINT uGender, 
										 long nBirthday, 
										 PCTSTR pszAddress, 
...
void CFlashIMClient::StartCheckNetworkStatusTask()
{
...
void CFlashIMClient::StartHeartbeatTask()
{
...
void CFlashIMClient::SendHeartbeatMessage()
{
	if(!m_bNetworkAvailable)
		return;
...
void CFlashIMClient::ModifyPassword(PCTSTR pszOldPassword, PCTSTR pszNewPassword)
{
	if(!m_bNetworkAvailable)
		return;
...
void CFlashIMClient::CreateNewGroup(PCTSTR pszGroupName)
{
	if(!m_bNetworkAvailable)
		return;
...
void CFlashIMClient::ModifyFriendMarkName(UINT friendID, PCTSTR pszNewMarkName)
{
    if (!m_bNetworkAvailable)
        return;
...
void CFlashIMClient::ResponseAddFriendApply(UINT uAccountID, UINT uCmd)
{
...
// 登录
void CFlashIMClient::Login(int nStatus/* = STATUS_ONLINE*/)
{
...
void CFlashIMClient::GetFriendList()
{
...
void CFlashIMClient::GetGroupMembers(int32_t groupid)
{
...
void CFlashIMClient::ChangeStatus(int32_t nNewStatus)
{
...
// 取消登录
void CFlashIMClient::CancelLogin()
{
	//m_ThreadPool.RemoveAllTask();
}

void CFlashIMClient::SetFindFriendWindow(HWND hwndFindFriend)
{
	m_hwndFindFriend = hwndFindFriend;
}

// 改变在线状态
void CFlashIMClient::ChangeStatus(long nStatus)
{
	if (IsOffline())
		return;
}
...
// 更新好友列表
void CFlashIMClient::UpdateBuddyList()
{
	if (IsOffline())
		return;
}
...
// 更新群列表
void CFlashIMClient::UpdateGroupList()
{
	if (IsOffline())
		return;
}
...
// 更新最近联系人列表
void CFlashIMClient::UpdateRecentList()
{
	if (IsOffline())
		return;
}
...
// 更新好友信息
void CFlashIMClient::UpdateBuddyInfo(UINT nUTalkUin)
{	
	if (IsOffline())
		return;
}
...
// 更新群成员信息
void CFlashIMClient::UpdateGroupMemberInfo(UINT nGroupCode, UINT nUTalkUin)
{
	if (IsOffline())
		return;
}
...
// 更新群信息
void CFlashIMClient::UpdateGroupInfo(UINT nGroupCode)
{
	if (IsOffline())
		return;
...
// 更新好友号码
void CFlashIMClient::UpdateBuddyNum(UINT nUTalkUin)
{
	//if (IsOffline())
	//	return;
...
// 更新群成员号码
void CFlashIMClient::UpdateGroupMemberNum(UINT nGroupCode, UINT nUTalkUin)
{
	//if (IsOffline())
	//	return;
...
// 更新群成员号码
void CFlashIMClient::UpdateGroupMemberNum(UINT nGroupCode, std::vector<UINT>* arrUTalkUin)
{
...
// 更新群号码
void CFlashIMClient::UpdateGroupNum(UINT nGroupCode)
{
	//if (IsOffline())
	//	return;
...
// 更新好友个性签名
void CFlashIMClient::UpdateBuddySign(UINT nUTalkUin)
{
	//if (IsOffline())
	//	return;
...
// 更新群成员个性签名
void CFlashIMClient::UpdateGroupMemberSign(UINT nGroupCode, UINT nUTalkUin)
{
	//if (IsOffline())
	//	return;
...
// 修改UTalk个性签名
void CFlashIMClient::ModifyUTalkSign(LPCTSTR lpSign)
{
...
// 更新好友头像
void CFlashIMClient::UpdateBuddyHeadPic(UINT nUTalkUin, UINT nUTalkNum)
{
	//if (IsOffline())
	//	return;
...
// 更新群成员头像
void CFlashIMClient::UpdateGroupMemberHeadPic(UINT nGroupCode, UINT nUTalkUin, UINT nUTalkNum)
{
	//if (IsOffline())
	//	return;
...
// 更新群头像
void CFlashIMClient::UpdateGroupHeadPic(UINT nGroupCode, UINT nGroupNum)
{
	//if (IsOffline())
	//	return;
...
// 更新群表情信令
void CFlashIMClient::UpdateGroupFaceSignal()
{
	if (IsOffline())
		return;
}
...
// 发送好友消息
BOOL CFlashIMClient::SendBuddyMsg(UINT nFromUin, const tstring& strFromNickName, UINT nToUin, const tstring& strToNickName, time_t nTime, const tstring& strChatMsg, HWND hwndFrom/* = NULL*/)
{
    return m_SendMsgThread.AddBuddyMsg(nFromUin, strFromNickName, nToUin, strToNickName, nTime, strChatMsg, hwndFrom);
}

// 发送群消息
BOOL CFlashIMClient::SendGroupMsg(UINT nGroupId, time_t nTime, LPCTSTR lpMsg, HWND hwndFrom)
{
	if (IsOffline())
		return FALSE;
...
// 发送临时会话消息
BOOL CFlashIMClient::SendSessMsg(UINT nGroupId, UINT nToUin, time_t nTime, LPCTSTR lpMsg)
{
	if (IsOffline())
		return FALSE;
...
BOOL CFlashIMClient::SendMultiChatMsg(const std::set<UINT> setAccountID, time_t nTime, LPCTSTR lpMsg, HWND hwndFrom/*=NULL*/)
{
	if (IsOffline())
		return FALSE;
	//群发消息
...
// 是否离线状态
BOOL CFlashIMClient::IsOffline()
{
	return (STATUS_OFFLINE == m_UserMgr.m_UserInfo.m_nStatus) ? TRUE : FALSE;
}

// 获取在线状态
long CFlashIMClient::GetStatus()
{
	return m_UserMgr.m_UserInfo.m_nStatus;
}

// 获取验证码图片
BOOL CFlashIMClient::GetVerifyCodePic(const BYTE*& lpData, DWORD& dwSize)
{
...
void CFlashIMClient::SetBuddyListAvailable(BOOL bAvailable)
{
	m_bBuddyListAvailable = bAvailable;
}
	
BOOL CFlashIMClient::IsBuddyListAvailable()
{
	return m_bBuddyListAvailable;
}

// 获取用户信息
CBuddyInfo* CFlashIMClient::GetUserInfo(UINT uAccountID/*=0*/)
{
...
// 获取好友列表
CBuddyList* CFlashIMClient::GetBuddyList()
{
	return &m_UserMgr.m_BuddyList;
}

// 获取群列表
CGroupList* CFlashIMClient::GetGroupList()
{
	return &m_UserMgr.m_GroupList;
}

// 获取最近联系人列表
CRecentList* CFlashIMClient::GetRecentList()
{
	return &m_UserMgr.m_RecentList;
}

// 获取消息列表
CMessageList* CFlashIMClient::GetMessageList()
{
	return &m_UserMgr.m_MsgList;
}

// 获取消息记录管理器
CMessageLogger* CFlashIMClient::GetMsgLogger()
{
	return &m_UserMgr.m_MsgLogger;
}

// 获取用户文件夹存放路径
tstring CFlashIMClient::GetUserFolder()
{
	return m_UserMgr.GetUserFolder();
}

// 获取个人文件夹存放路径
tstring CFlashIMClient::GetPersonalFolder(UINT nUserNum/* = 0*/)
{
	return m_UserMgr.GetPersonalFolder();
}

// 获取聊天图片存放路径
tstring CFlashIMClient::GetChatPicFolder(UINT nUserNum/* = 0*/)
{
	return m_UserMgr.GetChatPicFolder();
}

// 获取用户头像图片全路径文件名
tstring CFlashIMClient::GetUserHeadPicFullName(UINT nUserNum/* = 0*/)
{
	return m_UserMgr.GetUserHeadPicFullName(nUserNum);
}

// 获取好友头像图片全路径文件名
tstring CFlashIMClient::GetBuddyHeadPicFullName(UINT nUTalkNum)
{
	return m_UserMgr.GetBuddyHeadPicFullName(nUTalkNum);
}

// 获取群头像图片全路径文件名
tstring CFlashIMClient::GetGroupHeadPicFullName(UINT nGroupNum)
{
	return m_UserMgr.GetGroupHeadPicFullName(nGroupNum);
}

// 获取群成员头像图片全路径文件名
tstring CFlashIMClient::GetSessHeadPicFullName(UINT nUTalkNum)
{
	return m_UserMgr.GetSessHeadPicFullName(nUTalkNum);
}

// 获取聊天图片全路径文件名
tstring CFlashIMClient::GetChatPicFullName(LPCTSTR lpszFileName)
{
	return m_UserMgr.GetChatPicFullName(lpszFileName);
}

// 获取消息记录全路径文件名
tstring CFlashIMClient::GetMsgLogFullName(UINT nUserNum/* = 0*/)
{
	return m_UserMgr.GetMsgLogFullName();
}

// 判断是否需要更新好友头像
BOOL CFlashIMClient::IsNeedUpdateBuddyHeadPic(UINT nUTalkNum)
{
	return m_UserMgr.IsNeedUpdateBuddyHeadPic(nUTalkNum);
}

// 判断是否需要更新群头像
BOOL CFlashIMClient::IsNeedUpdateGroupHeadPic(UINT nGroupNum)
{
	return m_UserMgr.IsNeedUpdateGroupHeadPic(nGroupNum);
}

// 判断是否需要更新群成员头像
BOOL CFlashIMClient::IsNeedUpdateSessHeadPic(UINT nUTalkNum)
{
	return m_UserMgr.IsNeedUpdateSessHeadPic(nUTalkNum);
}

// 获取服务器时间
void CFlashIMClient::RequestServerTime()
{
	//m_IUProtocol.RequestServerTime();
}

time_t CFlashIMClient::GetCurrentTime()
{
...
void CFlashIMClient::OnNetworkStatusChange(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnHeartbeatResult(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnRegisterResult(UINT message, WPARAM wParam, LPARAM lParam)
{
    ::SendMessage(m_hwndRegister, message, wParam, lParam);	
}

void CFlashIMClient::OnLoginResult(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnUpdateUserBasicInfo(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnUpdateGroupBasicInfo(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnModifyInfoResult(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnRecvUserStatusChangeData(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnRecvAddFriendRequest(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnUserStatusChange(UINT message, WPARAM wParam, LPARAM lParam)
{
	CFriendStatus* pFriendStatus = (CFriendStatus*)lParam;
	if(pFriendStatus == NULL)
		return;
...
void CFlashIMClient::OnSendConfirmMessage(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnUpdateChatMsgID(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnFindFriend(UINT message, WPARAM wParam, LPARAM lParam)
{
	::PostMessage(m_hwndFindFriend, FMG_MSG_FINDFREIND, 0, lParam);
}

void CFlashIMClient::OnBuddyCustomFaceAvailable(UINT message, WPARAM wParam, LPARAM lParam)
{
	UINT uAccountID = (UINT)wParam;
	if(uAccountID == 0)
		return;
...
void CFlashIMClient::OnModifyPasswordResult(UINT message, WPARAM wParam, LPARAM lParam)
{
    ::PostMessage(m_hwndModifyPassword, message, wParam, lParam);
}

void CFlashIMClient::OnCreateNewGroupResult(UINT message, WPARAM wParam, LPARAM lParam)
{
	::PostMessage(m_hwndCreateNewGroup, message, wParam, lParam);
}

void CFlashIMClient::OnDeleteFriendResult(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnUpdateBuddyList(UINT message, WPARAM wParam, LPARAM lParam)
{
	::SendMessage(m_UserMgr.m_hCallBackWnd, message, wParam, lParam);	
}

void CFlashIMClient::OnUpdateGroupList(UINT message, WPARAM wParam, LPARAM lParam)
{
	//BOOL bSuccess = FALSE;
...
void CFlashIMClient::OnUpdateRecentList(UINT message, WPARAM wParam, LPARAM lParam)
{
	::SendMessage(m_UserMgr.m_hCallBackWnd, message, 0, 0);
}

void CFlashIMClient::OnBuddyMsg(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnGroupMsg(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnSessMsg(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnSysGroupMsg(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnStatusChangeMsg(UINT message, WPARAM wParam, LPARAM lParam)
{
	//UINT nUTalkUin = 0;
...
void CFlashIMClient::OnKickMsg(UINT message, WPARAM wParam, LPARAM lParam)
{
    ::PostMessage(m_UserMgr.m_hCallBackWnd, FMG_MSG_SELF_STATUS_CHANGE, 0, (LPARAM)STATUS_ONLINE);
}

void CFlashIMClient::OnScreenshotMsg(UINT message, WPARAM wParam, LPARAM lParam)
{
    ::PostMessage(m_UserMgr.m_hCallBackWnd, FMG_MSG_SCREENSHOT, wParam, lParam);
}

void CFlashIMClient::OnUpdateBuddyNumber(UINT message, WPARAM wParam, LPARAM lParam)
{
	//UINT nUTalkUin = 0;
...
void CFlashIMClient::OnUpdateGMemberNumber(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnUpdateGroupNumber(UINT message, WPARAM wParam, LPARAM lParam)
{
	//UINT nGroupCode = (UINT)wParam;
	//CGetUTalkNumResult* lpGetUTalkNumResult = (CGetUTalkNumResult*)lParam;
	//if (nGroupCode != 0 && lpGetUTalkNumResult != NULL)
...
void CFlashIMClient::OnUpdateBuddySign(UINT message, WPARAM wParam, LPARAM lParam)
{
	//UINT nUTalkUin = 0;
...
void CFlashIMClient::OnUpdateGMemberSign(UINT message, WPARAM wParam, LPARAM lParam)
{
	//UINT nGroupCode = (UINT)wParam;
	//UINT nUTalkUin = 0;
	//CGetSignResult* lpGetSignResult = (CGetSignResult*)lParam;
...
void CFlashIMClient::OnUpdateBuddyInfo(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnUpdateGMemberInfo(UINT message, WPARAM wParam, LPARAM lParam)
{
	//UINT nGroupCode = (UINT)wParam;
	//UINT nUTalkUin = 0;
...
void CFlashIMClient::OnUpdateGroupInfo(UINT message, WPARAM wParam, LPARAM lParam)
{
	//UINT nGroupCode = 0;
...
void CFlashIMClient::OnChangeStatusResult(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnTargetInfoChange(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnInternal_GetBuddyData(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CFlashIMClient::OnInternal_GetGroupData(UINT message, WPARAM wParam, LPARAM lParam)
{
	UINT nGroupCode = (UINT)wParam;
...
void CFlashIMClient::OnInternal_GetGMemberData(UINT message, WPARAM wParam, LPARAM lParam)
{
...
UINT CFlashIMClient::OnInternal_GroupId2Code(UINT message, WPARAM wParam, LPARAM lParam)
{
	CGroupInfo* lpGroupInfo = m_UserMgr.m_GroupList.GetGroupById(lParam);
	return ((lpGroupInfo != NULL) ? lpGroupInfo->m_nGroupCode : 0);
}
// 加载用户设置信息
void CFlashIMClient::LoadUserConfig()
{
...
void CFlashIMClient::SaveUserConfig()
{
	if(m_UserMgr.m_UserInfo.m_strAccount.empty())
		return;
...
BOOL CFlashIMClient::SetBuddyStatus(UINT uAccountID, long nStatus)
{
	//好友上线
	if(uAccountID!=m_UserMgr.m_UserInfo.m_uUserID)
	{
...
BOOL CFlashIMClient::SetBuddyClientType(UINT uAccountID, long nNewClientType)
{
    if (uAccountID != m_UserMgr.m_UserInfo.m_uUserID)
    {
...
long CFlashIMClient::ParseBuddyStatus(long nFlag)
{
...
void CFlashIMClient::GoOnline()
{
	if(m_bNetworkAvailable)
		Login();
}

void CFlashIMClient::GoOffline()
{
...
void CFlashIMClient::CacheBuddyStatus()
{
...
// 创建代理窗口
BOOL CFlashIMClient::CreateProxyWnd()
{
	WNDCLASSEX wcex;
	LPCTSTR szWindowClass = _T("FLASHIM_PROXY_WND");
	HWND hWnd;
...
// 销毁代理窗口
BOOL CFlashIMClient::DestroyProxyWnd()
{
...
LRESULT CALLBACK CFlashIMClient::ProxyWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	CFlashIMClient* lpFMGClient = (CFlashIMClient*)::GetWindowLong(hWnd, GWL_USERDATA);
	if (NULL == lpFMGClient)
