...
{	
    ...
    DWORD dwFlashIMProcessID = 0;
    do
    {
        if (_tcsicmp(pe32.szExeFile, _T("FlashIM.exe")) == 0)
        {
            dwFlashIMProcessID = pe32.th32ProcessID;
            break;
        }           
    ...
    //FlashIM.exe进程已经退掉，则认为升级成功
    if (dwFlashIMProcessID != 0)
    {
        HANDLE hAutoUpdateProcess = ::OpenProcess(PROCESS_ALL_ACCESS, FALSE, dwFlashIMProcessID);
