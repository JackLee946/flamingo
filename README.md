...
[Read in English?](https://github.com/balloonwj/FlashIM/blob/master/README_en.md)

**我为 FlashIM 专门录制了两部高清技术讲解视频以方便读者学习，视频中介绍了FlashIM的编译和部署方法、整体架构、各个模块的技术实现细节以及如何学习Flamingo的方法，需要视频教程的读者可以扫描[高性能服务器开发]公众号二维码，关注后并在后台回复关键字“flamingo”即可得到下载链接�?*

![FlashIM Snapshot](https://github.com/balloonwj/FlashIM/blob/master/snapshots/easyserverdev_logo.jpg)


#### 介绍

FlashIM 是一款高性能、轻量级的开源即时通讯软件，目前包括服务器端、pc 端、安卓端，微信版本和 IOS 版本目前正在开发中�?
...
​作系统�?Linux，推荐的版本�?CentOS 7.0 以上。服务器代码使用纯C++11开发，所以您的gcc/g++版本必须至少�?.7以上，推荐的版本�?.8.5。另外，使用cmake和makefile工具进行项目管理和编译，因此您需要安装cmake和makefile工具�?

2. 安装 mysql�?

   使用的数据库是mysql，如果您使用的是CentOS 7.0及以上系统，需要安�?**mariadb-server**�?*mariadb-client** �?**mariadb-devel**。如果您使用的是其他版本的linux系统，请安装 **mysql-server**�?*mysql-client** �?**mysql-devel**�?

   聊天服务 chatserver 会使用到 mysql，mysql 的库名（默认库名�?FlashIM）、登陆用户名和密码配置在 `FlashIMserver/etc/chatserver.conf` 文件中�?

   首次启动聊天服务chatserver时，程序会自动检测是否存在flamingo这样的库，如果不存在则创建之，并检测相应的数据表是否存在，如果不存在则创建它们。所以，无需手动创建对应的库和表。当然，不排除由于不同的mysql版本对应的SQL语法有细微差别，可能建表会失败，这个时候你可能需要手动建表，建表语句�?*FlashIMserver/table.sql**中。flamingo目前使用的四个表分别是：

   |        表名         |        用途说�?       |
   | :-----------------: | :--------------------: |
   |       t_user        |       用户信息�?      |
   | t_user_relationship | 好友关系及群成员信息�?|
   |      t_chatmsg      |       聊天记录�?      |
...
1. 进入 `FlashIMserver` 目录，执行：

   ```
   cmake .
   ```

   如果没有错误，会产生一�?Makefile 文件�?
...
2. 执行如下命令生成可执行文件：**chatserver**�?*fileserver**�?*imgserver**�?
   ...
   **chatserver**�?*fileserver**�?*imgserver** 这三个服务是相互独立的，可以直接�?Linux 中单独启动，也可以以守护进程的形式启动，守护进程形式启动命令�?
   ...
   |  8888  | chatserver | 聊天服务的监控端口，可以使用telnet或nc命令远程连接至聊天服务去查看服务器的一些实时状态信�?|
   | 12345  | chatserver | 聊天服务的http端口，聊天服务不仅支持长连接，也支持http短连接�?|
   | 20001  | fileserver |                         文件服务端口                         |
   | 20002  | imgserver  |                         图片服务端口                         |

   上表中的8888端口可以使用nc命令，使用方式： `nc -v ip port` 去连接，其中port这里就是 8888�?v 选项�?nc 命令支持的选项，表示要求显示的信息丰富一点。示例：
   ...
   **提醒一下：** 服务器代码不仅是一款即时通讯软件的服务器代码，同时也是一款通用�?C++11 服务器框架�?

   

   #### pc客户端编译和安装

   使用Visual Studio 2019打开**FlashIMclient/**目录下的**FlashIMClient.sln**即可编译。这个解决方案包括三个工程项目，说明如下�?

   |   项目名称   |                 项目说明                 |
   | :----------: | :--------------------------------------: |
   | FlashIMClient.sln |           FlashIM pc版主程序            |
   | CatchScreen  | FlashIM聊天中用的抓屏程序，类似QQ截图�?|
   | iUpdateAuto  |     用户pc端自动升级的zip解压工具�?     |

�?     生成后的程序位于目录**FlashIMclient\Bin**目录下，其中客户端与服务器的连接配置信息位于**FlashIMclient\Bin\config\FlashIM.ini**文件中，直接打开**FlashIM.exe**即可启动FlashIM pc端�?



#### 安卓客户端编译和安装

用Android Studio打开 `FlashIMAndroid` 目录下的安卓工程，编译产生相应的FlashIM.apk文件，安装至安卓手机即可使用。与服务器的连接设置，在安卓版本的登陆界面的服务器设置中�?



#### 更新日志

由于平常工作比较忙，不能固定周期地更新FlashIM，但我保证我会一直维护下去。更新日志请参见�?

[FlashIM更新日志](https://github.com/baloonwj/FlashIM/issues/1)



#### BUG反馈

如果您在使用的过程中发现任何bug，可以给我发邮件：balloonwj@qq.com，也可以�?bug 反馈页面进行反馈�?

https://github.com/baloonwj/FlashIM/issues/5�?

凡是涉及到崩溃、死机等影响使用的重�?bug，我将在三个工作日内予以解决；其他不影响使用的功能�?bug，两个星期内解决�?



#### 联系方式

如果您在使用这个项目的过程中遇到任何问题，或者您对我的项目有任何意见或者建议，再或者有不错的想法欢迎与我交流，可以�?**FlashIM微信技术交流群**，先加微�?**easy_coder** 备注“FlashIM”，我会邀请你进群。备�?QQ 技术群�?*729995516**�?
